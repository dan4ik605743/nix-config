...
let
  ...
  ## Gtk
  generated = pkgs.callPackage ./packages/generated.nix {inherit pywal;};
in
  { 
    home.username = "dan4ik";
      xsession = {
        enable = true;
        numlock.enable = true;
        initExtra = ''
          ...
          windowManager.i3 = with pywal.colors; {
            ...
            config = rec {
              ...
              startup = [
                {
                  command = "${pkgs.feh}/bin/feh --bg-scale /etc/nixos/system/packages/pywal/current";
                  always = false;
                  notification = false;
                }
                {
